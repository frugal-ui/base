...
export interface Component<ValueType> extends HTMLElement, Styleable {
     ...
     */
    useDefaultPadding: (paddingType?: PaddingTypes) => this;
    /**
     * Applies default gap.
     */
...
export function Component<ValueType>(
    ...
    component.useDefaultPadding = (paddingType = PaddingTypes.Outer) => {
        component.addToClass(`using-${paddingType}-padding`);
        return component;
    };
...
export function Accordion(label: string, ...children: Component<any>[]) {
    return Container(
        ...
        .useDefaultPadding(PaddingTypes.Inner);
}
...
export function AutoComplete<T>(
    ...
    return Div(
        ...
        input.setAttr('list', uuid).cssWidth('100%'),
    );
}
...
export function CommandShell(commands: Command[], mainView: Component<any>) {
    ...
    return VStack(
        ...
        AutoComplete(
            optionData,
            Input(new TextInputCfg(commandQuery, '⇧⌃P'))
                ...
                .access((self) =>
                    self.createBinding(inputBlurrer, () => self.blur()),
                )
